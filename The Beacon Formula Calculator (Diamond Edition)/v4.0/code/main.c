...
int main(void) {
    ...
    char inpbeta[64] = ""; //Input for beta
    ...
    SDL_FRect boxbeta = {20, 800, 500, 40}; //Box for beta

    SDL_FRect BeaconLayerFormula
        = {0, 0, 652, 367}; //BeaconLayerFormula image sizing

    SDL_FRect BeaconFormula
        = {653, 0, 1022, 575}; //BeaconFormula image sizing

    SDL_FRect CalculateBox
        = {20, 880, 60, 60}; //Calculate button
    
    SDL_FRect boxb_l = {20, 950, 500, 40}; //Box for beacon layer output
    SDL_FRect boxb = {20, 1000, 500, 40}; //Box for beacon output
    ...
    SDL_Surface *textSurfacebeta =
        TTF_RenderText_Blended(
            font,
            "beta = ",
            0,
            colour
        ); //Create surface with the rendered beta =  text
    ...
    SDL_Surface *textSurfaceINPbeta =
        TTF_RenderText_Blended(
            font,
            inpbeta,
            0,
            colour
        ); //Create surface with the rendered inpbeta text
    ...
    SDL_Texture *textTexturebeta = 
        SDL_CreateTextureFromSurface(
            renderer,
            textSurfacebeta
        ); //Create a texture from beta text surface
    ...
    SDL_Texture *textTextureINPbeta = 
        SDL_CreateTextureFromSurface(
            renderer,
            textSurfaceINPbeta
        ); //Create a texture from INPbeta text surface
    ...
    SDL_FRect OUTbeta = {30, 800, 45, 30}; //Output beta box
    SDL_FRect EQUAL = {30, 880, 30, 30}; //EQUAL
    SDL_FRect BL = {30, 950, 45, 30}; //b_l
    SDL_FRect B = {30, 1000, 45, 30}; //b

    float twh, thh, twL, thL, twW, thW, twm, thm, twbeta, thbeta;
    ...
    SDL_GetTextureSize(textTextureINPbeta, &twbeta, &thbeta);
    
    
    SDL_FRect INPh_0 = {150, 600, twh, thh}; //Input h box
    SDL_FRect INPL = {150, 650, twL, thL}; //Input L box
    SDL_FRect INPW = {150, 700, twW, thW}; //Input W box
    SDL_FRect INPm = {150, 750, twm, thm}; //Input m box
    SDL_FRect INPbeta = {150, 800, twbeta, thbeta}; //Input beta box
    ...
    SDL_DestroySurface(textSurfacebeta); //Free textSurfacebeta surface
    ...
    SDL_DestroySurface(textSurfaceINPbeta); //Free textSurfaceINPbeta surface
    ...
    while(running) {
        ...
        while(SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN) {
                ...
                } else if (mx >= boxbeta.x && mx <= boxbeta.x + boxbeta.w &&
                    my >= boxbeta.y && my <= boxbeta.y + boxbeta.h) {
            ...
            if (event.type == SDL_EVENT_TEXT_INPUT) {
                ...
                if (activeBox == 0) {
                    ...
                    strcat(inpbeta, event.text.text);
                }
            }
            ...
            if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN) {
                ...
                if (mx >= CalculateBox.x && mx <= CalculateBox.x + CalculateBox.w &&
                    my >= CalculateBox.y && my <= CalculateBox.y + CalculateBox.h) {
                        ...
                        int W = atoi(inpW);
                        int m = atoi(inpm);
                        int beta = atoi(inpbeta);
                        /*From string to an integer*/
                        if (L == 0 || W == 0 || m == 0) {
                            printf("Error Calculating: Area does not exist!\n");
                            sprintf(outb_l, "ERROR");
                            sprintf(outb, "ERROR");
                            ...
                            printf("Error Calculating: Negative Dimentions!\n");
                            sprintf(outb_l, "ERROR");
                            sprintf(outb, "ERROR");
                            ...
                            printf("Error Calculating: Beacon(s) do not exist!\n");
                            sprintf(outb_l, "ERROR");
                            sprintf(outb, "ERROR");
       ...
       textTextureINPL = 
        ...
        SDL_SetRenderDrawColor(renderer, 0, 0, 255, 255); //Blue
        SDL_RenderFillRect(renderer, &boxbeta); //Render boxbeta
        SDL_RenderTexture(
            renderer,
            textTexturebeta,
            NULL,
            &OUTbeta
        ); //Render beta texture to the renderer
       ...
       SDL_RenderTexture(
        ...
        SDL_RenderTexture(
            ...
            NULL,
            &INPbeta
        ); //Render INPbeta texture to the renderer
    ...
    SDL_DestroyTexture(textTexturebeta); //Free beta texture
    ...
    SDL_DestroyTexture(textTextureINPbeta); //Free INPbeta texture
